...
namespace Sistema_Desktop_P4
{
    public partial class FrmConsultarChamados : Form
    {
        public FrmConsultarChamados()
        {
            ...
            // Adiciona evento de duplo clique no DataGridView
            dataGridViewChamados.CellDoubleClick += DataGridViewChamados_CellDoubleClick;
        }
        ...
        private void DataGridViewChamados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                var chamado = (ChamadoDto)dataGridViewChamados.Rows[e.RowIndex].DataBoundItem;
                using (var form = new FrmDetalhesChamado(chamado))
                {
                    form.ShowDialog();
                }
                // Atualiza lista após fechar detalhes
                CarregarChamados();
            }
        }
    }
